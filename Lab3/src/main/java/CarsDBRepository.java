...
public class CarsDBRepository implements CarRepository{
    ...
    public List<Car> findByManufacturer(String manufacturerN) {
        ...
        try (PreparedStatement preStmt = con.prepareStatement("SELECT * FROM cars WHERE manufacturer = ?")) {
            ...
            logger.error("Database error while finding by manufacturer: ", e);
    ...
    public List<Car> findBetweenYears(int min, int max) {
        ...
        try (PreparedStatement preStmt = con.prepareStatement("SELECT * FROM cars WHERE year BETWEEN ? AND ?")) {
            ...
            logger.error("Database error while finding cars between years: ", e);
            System.err.println("Error DB " + e);
        }
        logger.traceExit(cars);
        return cars;
    ...
    public void update(Integer id, Car elem) {
        ...
        try (PreparedStatement preStmt = con.prepareStatement(
            ...
            int result = preStmt.executeUpdate();
            logger.trace("Updated {} records", result);
        } catch (SQLException e) {
            logger.error("Database error while updating car: ", e);
            System.err.println("Error DB " + e);
        }
        logger.traceExit();
    }
